...
function dates() {
  const start = new Date(Date.now() + 24 * 60 * 60 * 1000); start.setMinutes(0, 0, 0);
  const end = new Date(start.getTime() + 30 * 60 * 1000);
  const compact = (date: Date) => date.toISOString().replace(/[-:]/g, "").replace(/\.\d{3}/, "");
  return { start, end, compact };
}

export function CalendarActions({ title, compact = false }: { title: string; compact?: boolean }) {
  const download = () => {
    const { start, end, compact: format } = dates();
