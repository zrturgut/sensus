...
export const analyzeSensusInput = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    if (data.kind === "clarity") {
      ...
      if (data.text.trim().length < 60 || words < 15) return { is_sufficient: false as const, guidance_message: "It looks like you're just getting started! Share a bit about what's feeling tense, a goal you're stuck on, or what's cluttering your mind right now.", source: "demo" as const };
